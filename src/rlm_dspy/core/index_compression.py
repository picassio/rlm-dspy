...
def compress_index(index_path: Path) -> CompressionStats:
    ...
    - *.json files (except manifest) -> *.json.gz
    ...
    # Files to keep uncompressed (for quick reading)
    keep_uncompressed = {"manifest.json", "config.json"}
    ...
    # Compress JSON files (except config/manifest which should stay readable)
    for json_file in index_path.glob("*.json"):
        if json_file.name in keep_uncompressed:
            # Keep uncompressed for quick reading
...
def is_compressed(index_path: Path) -> bool:
    ...
    # Files we keep uncompressed (shouldn't affect compression status)
    keep_uncompressed = {"manifest.json", "config.json"}
    ...
    # Check for uncompressed files (excluding those we always keep)
    has_npy = any(index_path.glob("*.npy"))
    has_json = any(f for f in index_path.glob("*.json") if f.name not in keep_uncompressed)
